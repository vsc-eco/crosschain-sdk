...
```
@magi/core     pure math + op builders, no network, no framework
   ↑
@magi/sdk      pool/price/balance providers + quickSwap orchestrator
   ↑
@magi/widget   React <MagiQuickSwap/> and <magi-quickswap> web component
```

The dependency direction is one-way: `core` imports nothing from `sdk`/`widget`; `sdk` imports only `core`; `widget` imports both. Preserve this — putting network calls or framework code into `@magi/core` breaks its "zero deps, pure functions" contract.
...
Per-package (run inside a package dir, or `pnpm --filter @magi/core test`):

```bash
pnpm --filter @magi/core test
pnpm --filter @magi/sdk test -- tests/quickSwap.test.ts          # single file
pnpm --filter @magi/sdk test -- -t "referral"                    # by test name
```
...
Theming is pure CSS custom properties (`--magi-*`) scoped to `.magi-quickswap`. The dark theme lives at `@magi/widget/themes/altera-dark.css`.
...
Build is `tsup` emitting ESM + `.d.ts` only. `@magi/widget` copies CSS verbatim and inlines SVGs as data URLs via tsup loaders.
